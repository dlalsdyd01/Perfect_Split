...
struct CutPiece {
    ...
    var closedMesh: Mesh {
        var combined = body
        let offset = combined.vertices.count
        combined.vertices.append(contentsOf: cap.vertices)
        combined.triangles.append(contentsOf: cap.triangles.map { $0 + offset })
        return combined
    }
}
...
enum MeshCutter {
    static func cut(
        ...
        with plane: Plane,
        epsilon: Float = 1e-5
    ) -> (positive: CutPiece, negative: CutPiece) {
        var posBody = Mesh()
        var negBody = Mesh()
        ...
        for ti in 0..<mesh.triangleCount {
            ...
            splitTriangle(
                ...
                pos: &posBody, neg: &negBody, cutEdges: &cutEdges
            )
        }
        ...
        for loop in loops {
            let cleanedLoop = clean(loop, normal: plane.normal, epsilon: epsilon * 10)
            guard cleanedLoop.count >= 3 else { continue }
            let orientedLoop = orient(cleanedLoop, toward: plane.normal)
            for (p0, p1, p2) in earClip(polygon: orientedLoop, normal: plane.normal) {
                negCap.appendTriangle(p0, p1, p2)
                posCap.appendTriangle(p0, p2, p1)
            }
        }

        return (
            positive: CutPiece(body: posBody, cap: posCap),
            negative: CutPiece(body: negBody, cap: negCap)
        )
    }
        ...
        while !remaining.isEmpty {
            ...
            while true {
                ...
                if let idx = remaining.firstIndex(where: { approxEqual($0.0, current, epsilon) }) {
                    ...
                    loop.append(edge.1)
                    current = edge.1
                } else if let idx = remaining.firstIndex(where: { approxEqual($0.1, current, epsilon) }) {
                    let edge = remaining.remove(at: idx)
                    loop.append(edge.0)
                    current = edge.0
                } else {
    ...
    private static func clean(
        ...
        var deduped: [SIMD3<Float>] = []
        for point in polygon {
            if let last = deduped.last, approxEqual(last, point, epsilon) {
                continue
            }
            deduped.append(point)
        }
        if deduped.count >= 2, approxEqual(deduped[0], deduped[deduped.count - 1], epsilon) {
            deduped.removeLast()
        }
        ...
        while changed, cleaned.count >= 3 {
            ...
            for i in 0..<cleaned.count {
                let previous = cleaned[(i - 1 + cleaned.count) % cleaned.count]
                let current = cleaned[i]
                let next = cleaned[(i + 1) % cleaned.count]

                if approxEqual(previous, current, epsilon) || approxEqual(current, next, epsilon) {
                    cleaned.remove(at: i)
                    changed = true
                    break
                }

                let turn = simd_cross(current - previous, next - current)
                let edgeScale = max(
                    simd_length(current - previous) * simd_length(next - current),
                    epsilon
                )
                if abs(simd_dot(turn, normal)) <= epsilon * edgeScale * 20 {
                    cleaned.remove(at: i)
                    changed = true
                    break
                }
    ...
    private static func orient(
        _ polygon: [SIMD3<Float>],
        toward normal: SIMD3<Float>
    ) -> [SIMD3<Float>] {
        guard polygon.count >= 3 else { return polygon }

        var areaNormal: SIMD3<Float> = .zero
        for i in 0..<polygon.count {
            let current = polygon[i]
            let next = polygon[(i + 1) % polygon.count]
            areaNormal += simd_cross(current, next)
        }

        if simd_dot(areaNormal, normal) < 0 {
